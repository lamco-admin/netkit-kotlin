...
class ConfigurationGeneratorTest {
    ...
    fun `generate configuration for enterprise`() {
        ...
        assertTrue(config.securityConfig.encryptionStandard.contains("Enterprise", ignoreCase = true) ||
    ...
    fun `high security uses WPA3`() {
        ...
        assertTrue(config.securityConfig.encryptionStandard.contains("WPA3", ignoreCase = true) ||
    ...
    fun `channel plan includes 5GHz for modern networks`() {
        ...
        assertTrue(config.channelPlan.band.contains("5", ignoreCase = true) ||
                   config.channelPlan.band.contains("dual", ignoreCase = true) ||
                   config.channelPlan.channelAssignments.values.any { it > 14 })
    }
    ...
    fun `channel plan avoids DFS channels for simple setups`() {
        ...
        val hasDfs = config.channelPlan.channelAssignments.values.any { it in dfsChannels }
    ...
    fun `office has separate guest network`() {
        ...
        assertTrue(config.ssidConfigs.size >= 2)
        assertTrue(config.ssidConfigs.any {
    ...
    fun `high device count recommends multiple APs`() {
        ...
        assertTrue(config.recommendations.any {
